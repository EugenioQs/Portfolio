# Eugenio Quintero — Data Analyst Portfolio

Personal portfolio website showcasing my data analytics projects. Built with HTML, CSS, and JavaScript — no framework, no build step.

**Live site:** https://eugenioqs.github.io/Portfolio/

## Projects

| Project | Tools | Description |
|---|---|---|
| Olist E-Commerce Analysis — Brazil Commerce Pulse | Python, MySQL, Power BI | End-to-end analysis of 100K+ orders from Brazil's largest e-commerce platform — Python and Pandas for cleaning, MySQL for business metrics, Power BI dashboard covering R$ 13.2M in revenue across 37 states |
| Ecommerce Data Warehouse — S3 & Redshift | Python, AWS S3, Redshift, Power BI | Cloud data pipeline for a UK retailer — star schema modeling, S3 data lake, Redshift Serverless warehouse (406k rows), and a 4-page Power BI dashboard |
| E-Commerce Fashion Sales Analysis | Python, MySQL, Power BI | End-to-end pipeline for a fashion retailer across 4 countries — data generation, cleaning, MySQL storage, and a Power BI executive dashboard |
| Plato's Pizza — End-to-End Sales Analysis | Python, Power BI | Full data pipeline analyzing a year of pizza sales — cleaning, feature engineering, and an interactive multi-page Power BI dashboard |
| Road Accidents — Federal Highways | Power BI | Dashboard analyzing 60,531 accident records on Brazil's federal highways (2018–2020) |
| Data Cleaning in SQL | SQL, MySQL | Transforms raw Nashville housing data — standardizing formats, removing duplicates, restructuring for analysis |
| EDA on Layoffs Dataset | SQL, MySQL | Exploratory analysis of layoff trends by company, year, and funding stage |

## Stack

- **Languages:** SQL, Python
- **Databases:** MySQL, Amazon Redshift
- **Cloud:** AWS S3, boto3
- **BI Tools:** Power BI, Tableau
- **Libraries:** Pandas, SQLAlchemy
- **Frontend:** HTML, CSS, JavaScript

## Contact

- Email: qeugenios@gmail.com
- LinkedIn: https://www.linkedin.com/in/eugenioqs/
- GitHub: https://github.com/EugenioQs
